...
func weightedWordMapping(words []string, weights []int) string {
	// Created byte array for kept the ans
	var result []byte

	for _, word := range words {
		sum := 0
		
		// 1. Calculate the character weight
		...
		// 3. Reverse character to appended the result
		mappedChar := byte('z' - mod)
		result = append(result, mappedChar)
	}
